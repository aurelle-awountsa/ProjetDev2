        ...
        if email_check:
            if password_check:
                ...
                messagebox.showerror("Error",
                                     "Le mot de passe doit avoir (min 8 characteres ,majuscule et minuscule , chiffre et symbole)")
...
def insertion():
    ...
    if matricule_value != "" and name_value != "" and email_value != "" and password_value != "":
        try:
            ...
            myCursor.execute("INSERT INTO enqueteur VALUES (?,?,?,?,?)",
                             [matricule_value, name_value, email_value, password_value, gender_value])
            conn.commit()
...
Button(sign, text='Avez-vous déjà un compte?', command=loginForm, font=('Comic Scan Ms', 10, 'italic')).place(x=215,
                                                                                                              y=510)
